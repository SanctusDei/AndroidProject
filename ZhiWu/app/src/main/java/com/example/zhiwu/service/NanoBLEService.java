...
public class NanoBLEService extends Service {
    ...
    public static final String ACTION_SCAN_STARTED = "com.ubi.UbiNIRS.bluetooth.service.ACTION_SCAN_STARTED";
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            // 首先检查状态码，如果不是 SUCCESS，说明底层已经崩了
            if (status != BluetoothGatt.GATT_SUCCESS) {
                Log.e(TAG, "GATT连接发生异常，状态码: " + status);
                broadcastUpdate(KSTNanoSDK.ACTION_GATT_DISCONNECTED);
                gatt.close(); // 必须彻底释放，否则下次必卡死
                return;
            }

            if (newState == BluetoothProfile.STATE_CONNECTED) {
                if (debug) Log.i(TAG, "Connected to GATT server (物理链路已通).");

                // 【关键修复】立即通知 Activity 物理连接成功，让 UI 状态改变
                broadcastUpdate(KSTNanoSDK.ACTION_GATT_CONNECTED);

                // 【关键修复】延迟 600ms 后再发现服务，大幅提高 NIRScan 握手成功率
                new android.os.Handler(android.os.Looper.getMainLooper()).postDelayed(() -> {
                    if (KSTNanoSDK.mBluetoothGatt != null) {
                        boolean result = KSTNanoSDK.mBluetoothGatt.discoverServices();
